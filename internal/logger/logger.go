...
// maxSize 单位 MB (0 使用 lumberjack 默认 100MB), maxBackups 为保留的旧文件
// 数量上限, maxAge 为旧文件保留天数, 后两者为 0 表示不限制
func NewRotateWriter(path string, maxSize, maxBackups, maxAge int, compress bool) io.WriteCloser {
	return &lumberjack.Logger{
		...
		MaxAge:     maxAge,
		Compress:   compress,
		LocalTime:  true, // 轮转文件名时间戳使用本地时间, 与日志内容一致
	}
}

func Debug(msg string, args ...any) {
	global.Load().Debug(msg, args...)
}

func Info(msg string, args ...any) {
	global.Load().Info(msg, args...)
}

func Warn(msg string, args ...any) {
	global.Load().Warn(msg, args...)
}

func Error(msg string, args ...any) {
	global.Load().Error(msg, args...)
}
